...
All responses are returned in JSON format. Successful responses include the requested data, while error responses include an error message and relevant details.
...
```bash
curl -X GET https://api.plexe.ai/v1/models?limit=10&offset=20 \
  -H "x-api-key: YOUR_API_KEY_HERE"
```
