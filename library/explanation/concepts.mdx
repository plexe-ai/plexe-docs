...
* **`DatasetGenerator`:** A class (`plexe.DatasetGenerator`) can be used to wrap existing data or define parameters for synthetic data generation (using an LLM provider). Pass instances of this class in the `datasets` list if needed.