...
This callback is automatically added when you set `chain_of_thought=True` in `model.build()`. It captures the detailed reasoning steps and decision-making process of the AI agents.
...
# Log to file
file_handler = logging.FileHandler("plexe_build.log")
file_handler.setLevel(logging.DEBUG)
file_handler.setFormatter(logging.Formatter('%(asctime)s - %(name)s - %(levelname)s - %(message)s'))

# Configure Plexe logging
plexe.configure_logging(
    level=logging.DEBUG,
