...
```bash
curl -X GET https://api.plexe.ai/v1/jobs \
...
```bash
curl -X GET https://api.plexe.ai/v1/jobs/{job_id} \
  -H "x-api-key: YOUR_API_KEY"
```
...
```json
{
  ...
  "logs_url": "https://api.plexe.ai/v1/jobs/job_abc123/logs"
}
```
...
```json
{
  ...
  "result": {
    ...
    "endpoint": "https://api.plexe.ai/v1/predict/deployment_ghi789",
    "status": "READY"
  },
  "logs_url": "https://api.plexe.ai/v1/jobs/job_def456/logs"
}
```
...
```json
{
  ...
  "logs_url": "https://api.plexe.ai/v1/jobs/job_mno345/logs"
}
```
...
```bash
curl -X GET https://api.plexe.ai/v1/jobs/{job_id}/logs \
  -H "x-api-key: YOUR_API_KEY"
```
...
```bash
curl -X POST https://api.plexe.ai/v1/jobs/{job_id}/cancel \
  -H "x-api-key: YOUR_API_KEY" \
...
```bash
curl -X POST https://api.plexe.ai/v1/webhooks \
  -H "x-api-key: YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
