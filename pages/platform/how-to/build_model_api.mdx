...
Make a `POST` request to the endpoint for creating models, typically including the desired model name in the path (e.g., `/v0/models/{model_name}`).
...
try:
    response = requests.post(
        f"{base_url}/v0/models/{model_name}",
        headers=headers,
        json=build_payload
    )
