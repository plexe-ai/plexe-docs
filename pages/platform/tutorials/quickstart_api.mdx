...
1.  **Account:** You need a Plexe Platform account. Sign up at [console.plexe.ai](https://console.plexe.ai).
...
try:
    ...
    confirm_response = requests.post(
        f"{base_url}/data/use-upload",
        headers=headers,
        json={"upload_id": temp_upload_id, "filename": file_name}
    )
...
<Note>
Data uploads use a two-step process: first get a pre-signed URL, then upload to that URL directly. This allows for secure and efficient handling of large files.
</Note>
...
build_payload = {
    "goal": "Predict house prices in USD based on features like sqft, beds, baths.",
    "upload_id": dataset_reference, # Use upload_id or URL from previous step
...
if model_id:
    ...
    while status in ["pending", "processing", "building"]:
        ...
        try:
            ...
            status_url = f"{base_url}/models/{m_name}/{m_version}/status"
...
if model_id and status == "completed":
    ...
    try:
        ...
        infer_url = f"{base_url}/models/{m_name}/{m_version}/infer"
        response = requests.post(infer_url, headers=headers, json=inference_payload)
        response.raise_for_status()
        prediction_result = response.json()
        prediction = prediction_result.get("predictions", {})
